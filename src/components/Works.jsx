  ...
  return (
    ...
    >
      <Tilt
        ...
        className='flex w-full items-center justify-center rounded-xl bg-white/5 shadow-lg transition-all duration-300 hover:shadow-[0_0_20px_white] sm:w-[360px]'
      >
        <div className='h-full rounded-2xl bg-[#0e0e1c] p-5'>
          {/* Project Image */}
          <div className='relative h-[230px] w-full overflow-hidden rounded-xl'>
            <img
              src={image}
              alt='project'
              className='h-full w-full transform rounded-xl object-cover transition-transform duration-500 hover:scale-105'
            />
            {/* GitHub Icon */}
            <div className='absolute inset-0 m-3 flex justify-end'>
              <div
                onClick={() => window.open(source_code_link, '_blank')}
                className='flex h-10 w-10 cursor-pointer items-center justify-center rounded-full bg-black bg-opacity-60 backdrop-blur-sm transition-transform duration-300 hover:scale-110'
              >
